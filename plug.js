...
(function(){
    ...
    function socket() {
        ...
        sock.onclose = function() {
            console.log('[Takimoyuncu' + version + '] girişler kapandı yeniden acılıyor!');
        ...
        for (var i in emotes) {
            if (settings[i]) {
                for (var j in emotes[i]) {
                    ...
                    var k = ':' + j.toLowerCase() + ':';
                    if (c.indexOf(k) > -1) b = b.replace(new RegExp(k,'gi'),'<div class="custom-emote" ' + (emotes[i][j].hidden ? 'Hidden Emote!' : 'title=":' + j + ':" ') + 'style="background-image:url(' + emotes[i][j].url + ');width:' + emotes[i][j].width + ';height:' + emotes[i][j].height + ';"></div>');
                }
            }
        }
        a.html(b);
